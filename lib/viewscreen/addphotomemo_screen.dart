...
class _Controller {
  ...
  void getPhoto(PhotoSource source) async {
    try {
      ...
      if (Constant.devMode) print('======= Failed to get picture $e');
      showSnackBar(
        context: state.context,
        message: 'Failed to get pic: $e',
      );
    }
  }
  ...
  void save() async {
    ...
    if (state.photo == null) {
      showSnackBar(context: state.context, message: 'Photo is not selected');
      return;
    }
    currentState.save();
    ...
    try {
      ...
      tempMemo.imageLabels = await GoogleMlController.getImageLabels(photo: state.photo!);
